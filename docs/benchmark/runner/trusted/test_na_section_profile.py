...
def test_scope_requires_tests_and_protects_harness(tmp_path: Path):
    _git(tmp_path, "init")
    ...
    assert (
        run_na_section_profile.validate_candidate_scope(tmp_path, baseline)
    ...
    assert run_na_section_profile.validate_candidate_scope(tmp_path, baseline) is None

    protected = tmp_path / "docs/benchmark/runner"
    protected.mkdir(parents=True)
    (protected / "tamper.py").write_text("TAMPER = True\n")
    assert (
        run_na_section_profile.validate_candidate_scope(tmp_path, baseline)
        == "candidate modified protected benchmark runner assets"
    )
