#!/usr/bin/env python3
"""Perfil trusted para la ronda «organización actual segura»."""
...
def ensure_verifier_image() -> None:
    run_rat_default.command(
        [
            ...
            VERIFIER_IMAGE,
            "--file",
            str(TRUSTED_ROOT / "Dockerfile.organization-current-verifier"),
            str(TRUSTED_ROOT),
        ]
    )
    ...
    return run_rat_default.command(
        [
            ...
            *run_rat_default.CONTAINER_SECURITY_ARGS,
            ...
            "--mount",
            f"type=bind,src={TEST_FILE},dst=/opt/rat/test_organization_current_contract.py,readonly",
            "--env",
            "HOME=/tmp",
            "--env",
            ...
            "-p",
            "no:cacheprovider",
            "/opt/rat/test_organization_current_contract.py",
        ],
        check=False,
    )
...
def validate_candidate_scope(workspace: Path, baseline_commit: str) -> str | None:
    ...
    protected = next(
        (path for path in paths if path.startswith(PROTECTED_PREFIXES)), None
    )
    if protected:
        return f"candidate modified protected asset: {protected}"
    return None
...
def run_profile(run_id: str, workspace: Path, evidence: Path) -> int:
    ...
    if base_code != 0:
        ...
        _append_log(
            log_path, "SKIP organization_current_contract: rat-default did not pass\n"
        )
        return base_code

    try:
        baseline_commit = json.loads(
        ...
        if result.returncode == 0 and scope_failure is None:
            task_status, exit_code = "PASS", 0
            report["status"] = "PASS"
            report["trustedChecksPassed"] = True
            report["error"] = None
        elif 1 <= result.returncode <= 5:
            task_status, exit_code = "FAIL", 1
            report["status"] = "FAIL"
            report["trustedChecksPassed"] = False
            report["error"] = None
        else:
            task_status, exit_code = "HARNESS_ERROR", 2
